...
import { getRoleForEmail, getAgentNameForEmail, getBranchForEmail , resolveRole } from "@/lib/roles";
import { generateTodoTickets } from "@/lib/csrOps";
...
import {
  ...
  User, Mail, MapPin, CalendarDays, BadgeCheck, Edit3, X, Download, ListTodo, ArrowRight,
} from "lucide-react";
...
function EmployeeMistakePage(){
  ...
  const { email, role: authRole } = useAuth();
  // A regular CSR agent may only view their OWN profile, not other agents'.
  const role = resolveRole(email, authRole);
  const selfView = role.id === "csr";
  if (selfView) {
    const ownName = getAgentNameForEmail(email);
    if (ownName && name.toLowerCase() !== ownName.toLowerCase()) {
      return <Navigate to="/csr/mistake/$employeeId" params={{ employeeId: ownName.replace(/\s+/g, "_") }} />;
    }
  }
  ...
  const todoBranch = getBranchForEmail(email) || "Nashville";
  const todoCount = useMemo(() => generateTodoTickets(name, todoBranch, 18).length, [name, todoBranch]);
  ...
  const visibleTabs = TABS.filter(t => !(selfView && t.id === "da"));
    ...
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 w-full max-w-screen-2xl mx-auto px-8 py-8">
        ...
        <div className="panel p-5 mb-5">
          ...
          {/* To-Do quick link — tickets not yet set to Need Scheduling */}
          <Link to="/m/$module/$submodule" params={{ module: "tickets", submodule: "todo-list" }}
            className="mt-4 flex items-center justify-between gap-3 px-4 py-3 rounded-lg border border-yellow-500/25 bg-yellow-500/5 hover:bg-yellow-500/10 transition-colors group">
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-lg bg-yellow-500/15 flex items-center justify-center shrink-0">
                <ListTodo className="h-5 w-5 text-yellow-300" />
              </div>
              <div>
                <p className="text-sm font-semibold flex items-center gap-2">
                  To-Do List
                  <span className="px-2 py-0.5 rounded-full text-xs bg-yellow-500/20 text-yellow-300 border border-yellow-500/30">{todoCount}</span>
                </p>
                <p className="text-xs text-muted-foreground mt-0.5">Tickets from {todoBranch} not yet set to Need Scheduling</p>
              </div>
            </div>
            <ArrowRight className="h-4 w-4 text-muted-foreground group-hover:text-yellow-300 transition-colors" />
          </Link>
        </div>
