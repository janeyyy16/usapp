...
import { canAccessSubmodule, resolveRole, getAgentNameForEmail } from "@/lib/roles";
...
import { CSRTodoList } from "@/components/CSRTodoList";
...
function SubModule() {
  ...
  const data = Route.useLoaderData() as { mod: any; sub: any } | undefined;
  const mod = data?.mod;
  const sub = data?.sub;
  const location = useLocation();
  if (!ready) return null;
  if (!email) return <Navigate to="/landing" />;
  if (!mod || !sub) return <Navigate to="/home" />;
  // Role guard — block deep-links to submodules the role can't access
  if (!canAccessSubmodule(email, mod.slug, sub.slug, role)) {
    return <Navigate to="/home" />;
  }
  // Regular CSR agents see only their personal tracker, not the team dashboard.
  if ((sub as any).custom === "csr-dashboard" && resolveRole(email, role).id === "csr") {
    const agentName = getAgentNameForEmail(email);
    if (agentName) {
      return <Navigate to="/csr/mistake/$employeeId" params={{ employeeId: agentName.replace(/\s+/g, "_") }} />;
    }
  }
  ...
  return (
    <>
      ...
      {sub.slug === "overall-status"
        ...
        : sub.slug === "todo-list"
        ? ((resolveRole(email, role).id === "csr" || resolveRole(email, role).id === "csr_tl")
            ? <CSRTodoList mod={mod} sub={sub} />
        ...
        ? (resolveRole(email, role).id === "csr_tl"
