...
import { canAccessModule, resolveRole } from "@/lib/roles";
...
function Home() {
  ...
  const roleDef = resolveRole(email, role);
  const visibleModules = MODULES.filter((m) => canAccessModule(email, m.slug, role));
  return (
    <>
      <AppHeader />
      <main className="max-w-[1400px] mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-muted-foreground">Signed in as <span className="text-foreground/80">{email}</span> · {roleDef.label}</p>
        </div>
