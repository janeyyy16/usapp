...
import { buildTicketDetailFromTodo } from "@/lib/csrOps";
import { TICKETS } from "@/lib/ticketData";
import { useState, useEffect, useMemo } from "react";
import { AppHeader } from "@/components/Header";
import { TicketSidebar } from "@/components/TicketSidebar";
import { TicketPartTransaction } from "@/components/TicketPartTransaction";
...
// Build a full detail record from any Ticket List entry so every ticket opens.
function buildTicketDetailFromList(ticketNo: string): TicketData | null {
  const t = TICKETS.find((x) => x.ticketNo === ticketNo);
  if (!t) return null;
  return {
    ...DEFAULT_TICKET,
    ticketNo: t.ticketNo,
    account: t.account || "GSL00002",
    warranty: t.warranty || "IW",
    product: t.productType || "Appliance",
    tat: `${t.aging ?? 0}d`,
    status: t.status,
    schedule: t.schedule || "N/A",
    contact: t.contact || "N",
    location: t.location || "",
    firstName: t.firstName || (t.customer || "").split(" ")[0] || "",
    lastName: t.lastName || (t.customer || "").split(" ").slice(1).join(" ") || "",
    address: t.address || "",
    city: t.city || "",
    state: t.state || "",
    zip: t.zip || "",
    homePhone: t.phone || "",
    cellPhone: t.secondPhone || t.phone || "",
    email: t.email || "",
    brand: t.manufacturer || "",
    model: t.model || "",
    serialNo: t.serial || "",
    productCategory: t.productType || "",
    purchaseDate: t.purchaseDate || "",
    warrantyType: t.warranty === "OOW" ? "Out of warranty" : "In warranty",
    claimCompany: t.claimCompany || t.account || "",
    accountNo: t.account || "",
    callNo: t.ticketNo,
    callType: t.type || "SMS",
    callStatus: t.status,
    postingDate: t.created || "",
    scheduleDate: t.schedule || "",
    schedulePeriod: "N/A",
    technician: t.technician || "",
    problemDescription: t.internalNote || `${t.productType || "Appliance"} service request for ${t.customer}.`,
    customerNotes: [
      { date: t.created || "", notes: `Ticket opened from the ticket list for ${t.customer}.`, by: "SYSTEM" },
    ],
  };
}
...
function TicketDetailsPage() {
  ...
  const ticket = useMemo(
    () => TICKET_DATA[ticketNo] || buildTicketDetailFromTodo(ticketNo) || buildTicketDetailFromList(ticketNo) || undefined,
    [ticketNo]
  );

  const addServicerNote = () => {
    if (newServicerNote.trim() && ticket) {
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 bg-slate-950 py-6">
        ...
        {/* Tabs removed — navigation handled by the collapsible side menu */}
        ...
        <div className="max-w-6xl mx-auto px-6 py-6">
          ...
          {activeTab === "tracking" && (
            <div className="space-y-8">
              {/* Related Tickets */}
              <div id="section-related-tickets" className="scroll-mt-24">
              ...
              {/* Visit Log */}
              <div id="section-visit-log" className="scroll-mt-24">
              ...
              {/* Part Transaction */}
              <TicketPartTransaction ticketStatus={ticket?.status ?? ""} ticketNo={ticketNo} />
              {/* Comments */}
              <div>
