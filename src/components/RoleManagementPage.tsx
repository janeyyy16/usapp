...
/**
 * Bulk secondary-role assignment grid — one row per company user, one
 * checkbox column per assignable role (ROLE_OPTIONS, the same list the
 * individual user edit page's "User Type" multi-select uses). A checked box
 * means that role is held in extra_roles; the primary role (shown as its
 * own read-only column) is NOT editable here — changing someone's primary
 * role stays a deliberate, one-at-a-time action on their own profile page,
 * since it drives RLS and is a bigger deal than granting an additional
 * permission. This page only ever touches extra_roles.
 *
 * Formerly named "Accessibility Management" — renamed to Role Management
 * since that name now belongs to a different page (the Role x Module/
 * Submodule access matrix, see AccessibilityManagementPage.tsx).
 */
export function RoleManagementPage({ mod, sub }: Props) {
  ...
  return (
    <main className="flex-1 bg-slate-950 py-6">
      <div className="max-w-[1600px] mx-auto px-6">
        ...
        {/* max-h + overflow-auto makes this panel its OWN scroll container
            (both axes) instead of relying on the page's scroll — an
            overflow-x-auto-only wrapper forces overflow-y into an implicit
            scroll context too (per the CSS overflow spec), which made the
            sticky header anchor to the wrong frame and "stick" at a
            mid-scroll offset instead of the true top. With its own capped
            height, `top-0` below is relative to this panel's real scrollport. */}
        {/* p-0 overrides .panel's default padding — that padding sat inside
            this scroll container, above the sticky header, letting a sliver
            of the previous row peek through before the header fully caught.
            Cell-level px-3 py-2 already gives the table its own breathing
            room, matching how AdminUserManagementPage's own scrollable table
            wrapper avoids .panel padding for the same reason. */}
        <div ref={tableScrollRef} className="panel overflow-auto max-h-[70vh] p-0">
          <table className="min-w-full text-sm">
            <thead>
              <tr className="border-b border-white/10 text-left text-[10px] uppercase tracking-wide text-slate-500">
                <th className="px-3 py-2 whitespace-nowrap sticky left-0 top-0 z-20 bg-slate-950">Name</th>
                <th className="px-3 py-2 whitespace-nowrap sticky top-0 z-10 bg-slate-950">Primary Role</th>
                {ROLE_OPTIONS.map((r) => (
                  <th key={r.value} className="px-2 py-2 text-center whitespace-nowrap font-normal sticky top-0 z-10 bg-slate-950">
                    {r.label}
                  </th>
                ))}
              </tr>
