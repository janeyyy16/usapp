/**
 * External Fill W-9 — the no-login counterpart to FillW9Page.tsx, opened
 * from the link ReportHRDaily.tsx's "Send W-9 Request" panel generates when
 ...
 *
 * Same real-PDF overlay rendering as FillW9Page.tsx (identical field
 * rects). The PDF is built entirely client-side via the same pure
 * fillW9Pdf used by the logged-in flow, then POSTed already-finished to the
 * server bridge, which uploads it and notifies HR — no DM step here since
 * there's no sender profile.
 */
...
// Same field rectangles as FillW9Page.tsx — see that file's header comment
...
export function ExternalFillW9Page({ docId }: Props) {
    ...
    (async () => {
      ...
      try {
        ...
        if (!document || document.documentType !== "w9") {
          ...
          setForm((prev) => ({ ...prev, ...existing, name: existing.name || document.recipientName || "" }));
        }
      } catch (err) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [pdfjsLib, bytes] = await Promise.all([import("pdfjs-dist/legacy/build/pdf.mjs"), loadBlankW9Bytes()]);
