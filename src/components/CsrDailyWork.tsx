...
export function CsrDailyWork({ mod, sub }: Props) {
  ...
  const rows = useMemo(() => {
    let r = ALL_ROWS;
    if (csr) r = r.filter(x => x.csr === csr);
    if (startDate) r = r.filter(x => x.actionDate >= startDate);
    if (endDate) r = r.filter(x => x.actionDate <= endDate);
    if (csrOnly) r = r.filter(x => x.csrOnly);
    return r;
  }, [csr, csrOnly, endDate, startDate]);
