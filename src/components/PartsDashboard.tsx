...
const ALL_PO_ORDERS = generatePOOrders(120);
...
export function PartsDashboard({ mod, sub }: { mod: ModuleDef; sub: SubModuleDef }) {
  ...
  const distBreakdown = useMemo(() => {
    ...
    ALL_PO_ORDERS.forEach(o => { map[o.dist] = (map[o.dist] || 0) + 1; });
  ...
  const branchData = useMemo(() => {
    const map: Record<string, { collections: number; receives: number }> = {};
    agents.forEach((a: any) => {
      const b = a.branch || "Other";
      if (!map[b]) map[b] = { collections: 0, receives: 0 };
      map[b].collections += a.collections || 0;
      map[b].receives    += a.receives    || 0;
    });
    return Object.entries(map).map(([name, v]) => ({ name, ...v })).sort((a, b) => b.collections - a.collections).slice(0, 10);
  }, [agents]);
  ...
  const brandBreakdown = useMemo(() => {
    const map: Record<string, number> = {};
    ALL_PO_ORDERS.forEach(o => { map[o.brand] = (map[o.brand] || 0) + 1; });
  ...
  // Distributor table with fill rates
  const distTableData = useMemo(() => {
    const map: Record<string, { orders: number; inStock: number; outStock: number }> = {};
    ALL_PO_ORDERS.forEach(o => {
      if (!map[o.dist]) map[o.dist] = { orders: 0, inStock: 0, outStock: 0 };
      map[o.dist].orders++;
      if (o.availQty > 0) map[o.dist].inStock++; else map[o.dist].outStock++;
    });
    return Object.entries(map).map(([name, v]) => ({
      name, ...v, fillRate: Math.round(v.inStock / v.orders * 100),
    ...
    <div className="min-h-screen flex flex-col">
      <main className="flex-1 max-w-[1400px] mx-auto w-full px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-3 mb-4">
          {[
            { label: "Staff",          value: totals.agents,    color: "text-white" },
            { label: "Collections",    value: totals.collections,color:"text-blue-300" },
            { label: "Receives",       value: totals.receives,  color: "text-green-300" },
            { label: "Pending RA",     value: totals.pendingRA, color: "text-yellow-300" },
            { label: "Warnings",       value: totals.warnings,  color: totals.warnings > 0 ? "text-red-300" : "text-muted-foreground" },
            { label: "Pending PO",     value: pendingPO,        color: "text-purple-300" },
            { label: "Waiting Part",   value: waitingPart,      color: "text-orange-300" },
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
          ...
          <div className="panel p-4 lg:col-span-2">
            ...
            <ResponsiveContainer width="100%" height={200}>
              <LineChart data={trend10} margin={{ left: -10 }}>
                <XAxis dataKey="date" tick={{ fill: "#94a3b8", fontSize: 10 }} />
                <YAxis tick={{ fill: "#94a3b8", fontSize: 11 }} />
                <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6 }} />
                <Legend wrapperStyle={{ fontSize: 11, color: "#94a3b8" }} />
                <Line type="monotone" dataKey="collections" stroke="#3b82f6" strokeWidth={2} dot={{ r: 3 }} name="Collections" />
                <Line type="monotone" dataKey="receives"    stroke="#34d399" strokeWidth={2} dot={{ r: 3 }} name="Receives" />
                <Line type="monotone" dataKey="pendingRA"   stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} name="Pending RA" />
              </LineChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mb-4">
          <div className="panel p-4">
            <p className="text-sm font-semibold mb-4">Branch Performance (top 10)</p>
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={branchData} margin={{ left: -10 }}>
                <XAxis dataKey="name" tick={{ fill: "#94a3b8", fontSize: 9 }} angle={-20} textAnchor="end" height={46} />
                <YAxis tick={{ fill: "#94a3b8", fontSize: 11 }} />
                <Tooltip contentStyle={{ background: "#1e293b", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6 }} />
                <Legend wrapperStyle={{ fontSize: 11, color: "#94a3b8" }} />
                <Bar dataKey="collections" fill="#3b82f6" radius={[4, 4, 0, 0]} name="Collections" />
                <Bar dataKey="receives"    fill="#34d399" radius={[4, 4, 0, 0]} name="Receives" />
              </BarChart>
            </ResponsiveContainer>
          </div>
        ...
        <div className="panel p-0 overflow-hidden mb-4">
          ...
          <table className="w-full text-sm">
            <thead><tr className="border-b border-white/10 bg-white/5">
              {["Distributor","Orders","In Stock","Out of Stock","Fill Rate"].map(h =>
            ...
            <tbody>
              {distTableData.map((d, i) => (
                <tr key={d.name} className={`border-b border-white/5 hover:bg-white/5 ${i % 2 !== 0 ? "bg-white/[0.02]" : ""}`}>
                  ...
                  <td className="px-4 py-2 text-blue-300">{d.orders}</td>
                  <td className="px-4 py-2 text-green-300">{d.inStock}</td>
                  <td className="px-4 py-2 text-red-300">{d.outStock}</td>
                  <td className="px-4 py-2">
                    <div className="flex items-center gap-2">
                      <div className="h-1.5 w-16 rounded-full bg-white/10 overflow-hidden">
                        <div className="h-full rounded-full bg-blue-400" style={{ width: `${d.fillRate}%` }} />
                      </div>
                      <span className="text-xs text-muted-foreground">{d.fillRate}%</span>
                    </div>
                  </td>
                </tr>
              ))}
...
const FULL_PART_DISTS = ["(Part Dist.)","AIO","Electrolux","Encompass","Encompass-Birmingham / Montgomery","GE","LG","Marcone-Birmingham / Montgomery","Marcone-162468","Midea","Miele","NSA","OW","SB","Sharp","SP","Squaretrade"];
...
function PartOrderSection({ modSlug }: { modSlug: string }) {
  ...
  const filtered = useMemo(() => {
    return ALL_PO_ORDERS.filter(o => {
      ...
      if (technician && o.tech   !== technician) return false;
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-white/10 bg-white/5">
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase">Account</th>
              ...
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase">Wty</th>
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase">Tech Name</th>
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase">Part Dist.</th>
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase">Part No</th>
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase">Description</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase">ETA</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Request</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Inventory</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Reserve</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Avail.</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Return %</th>
              <th className="px-3 py-2.5 text-left text-xs text-muted-foreground uppercase border-l border-white/10">Note</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Reserve</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase">Reject</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase border-l border-white/10">Auto P/O</th>
              <th className="px-3 py-2.5 text-center text-xs text-muted-foreground uppercase">Manual P/O</th>
            </tr>
          </thead>
          <tbody>
            {filtered.length === 0
              ? <tr><td colSpan={19} className="px-4 py-10 text-center text-muted-foreground">No records match current filters.</td></tr>
              : filtered.map((o, i) => (
                <tr key={o.i} className={`border-b border-white/5 hover:bg-white/5 ${i % 2 !== 0 ? "bg-white/[0.02]" : ""}`}>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">{o.account}</td>
                  <td className="px-3 py-2.5 font-mono text-xs text-blue-400">{o.ticket}</td>
                  <td className="px-3 py-2.5 text-xs">
                    <span className={`px-1.5 py-0.5 rounded text-[10px] border ${o.status === "TR-Need PO" ? "bg-red-500/15 text-red-300 border-red-500/25" : o.status === "OP-Waiting for Part" ? "bg-yellow-500/15 text-yellow-300 border-yellow-500/25" : "bg-white/10 text-muted-foreground border-white/10"}`}>
                      {o.status}
                    </span>
                  </td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground">{o.wtyShort}</td>
                  <td className="px-3 py-2.5 text-xs whitespace-nowrap">{o.tech}</td>
                  <td className="px-3 py-2.5 text-xs">
                    <div className="flex items-center gap-1">
                      <select value={shipMethods[o.i] || ""} onChange={e => setShipMethods(p => ({ ...p, [o.i]: e.target.value }))}
                        className="glass-input text-[10px] py-0.5 px-1.5 rounded max-w-[120px]">
                        {SHIP_OPTIONS.map(s => <option key={s} value={s}>{s}</option>)}
                      </select>
                    </div>
                  </td>
                  <td className="px-3 py-2.5 font-mono text-xs text-blue-300">{o.partNo}</td>
                  <td className="px-3 py-2.5 text-xs">{o.desc}</td>
                  <td className="px-3 py-2.5 text-center text-xs text-muted-foreground">N/A</td>
                  <td className="px-3 py-2.5 text-center text-sm font-medium border-l border-white/5">{o.reqQty}</td>
                  <td className="px-3 py-2.5 text-center border-l border-white/5">
                    <span className={`text-sm font-semibold ${o.invQty > 0 ? "text-blue-300" : "text-muted-foreground"}`}>{o.invQty}</span>
                  </td>
                  <td className="px-3 py-2.5 text-center border-l border-white/5">
                    <span className={`text-sm ${o.resQty > 0 ? "text-blue-300" : "text-muted-foreground"}`}>{o.resQty}</span>
                  </td>
                  <td className="px-3 py-2.5 text-center border-l border-white/5">
                    <span className={`text-sm font-bold ${o.availQty > 0 ? "text-green-300" : "text-red-300"}`}>{o.availQty}</span>
                  </td>
                  <td className="px-3 py-2.5 text-center border-l border-white/5">
                    {o.returnPct > 0 ? <span className="text-xs text-yellow-300">{o.returnPct}%</span> : <span className="text-muted-foreground text-xs">—</span>}
                  </td>
                  <td className="px-3 py-2.5 text-xs text-muted-foreground border-l border-white/5">
                    {o.note ? <span className="text-xs text-blue-200/70">{o.note}</span> : ""}
                  </td>
                  <td className="px-3 py-2.5 text-center border-l border-white/5">
                  ...
                  <td className="px-3 py-2.5 text-center border-l border-white/5">
                    <select className="glass-input text-[10px] py-0.5 px-1.5 rounded w-20">
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <a href={`https://earlyrepair.com/Part/PartOrder`} target="_blank" rel="noopener noreferrer"
                      className="text-[10px] text-blue-400 hover:underline px-1.5 py-0.5 rounded border border-blue-500/30 bg-blue-500/10">
