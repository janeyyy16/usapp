...
// Turn ticket numbers (#-prefixed, must contain a digit) into links that open
// the ticket in a new tab. Pure-letter hashtags like #fun are ignored.
function renderMessageBody(body: string) {
  const TICKET_RE = /(#[A-Z0-9][A-Z0-9\-]*[0-9][A-Z0-9\-]*)/gi;
  const parts = body.split(TICKET_RE);
  return parts.map((part, i) => {
    if (TICKET_RE.test(part)) {
      const ticketNo = part.replace(/^#/, "").trim();
      return (
        <a key={i} href={`/ticket/${ticketNo}`} target="_blank" rel="noopener noreferrer"
          className="inline-flex items-center rounded bg-blue-500/20 px-1 py-0.5 font-mono text-[12px] text-blue-300 hover:bg-blue-500/35 hover:text-blue-200 transition-colors"
            ...
            {activeMessages.map((message) => {
              ...
              return (
                ...
                >
                  ...
                  <p className="whitespace-pre-wrap leading-6">{renderMessageBody(message.body)}</p>
                </div>
              );
            })}
            <div ref={endRef} />
