/**
 * Fill Damage, Part Loss, and Tool Penalty Commission Deduction Agreement —
 * opened from the deep link a Team Messenger message sends (see
 * ReportHRDaily.tsx's "Damage Agreement" tab "Send Request" flow). Same
 * architecture as FillLocationConsentPage.tsx: renders the REAL official
 ...
 * values directly onto that same real PDF via fillDamagePdf (there are no
 * AcroForm fields on this PDF at all — see damageFormTemplate.ts's header
 * comment) and sends the result back to HR.
 *
 * Genuine two-party document, same shape as Location Consent: only the
 * employee half (name, position/title, effective date, signature) is
...
import { uploadSignableDocumentSignature, uploadDamageForm, refreshStorageAuthToken } from "@/lib/firebase/storage";
...
// lines/labels on src/assets/DAMAGE.pdf — the exact numbers
// damagePdfFill.ts's draw coordinates were derived from. This PDF has no
...
export function FillDamagePage({ docId }: Props) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pdfUrl = await uploadDamageForm(companyId, form.employeeName, new Blob([pdfBytes as unknown as BlobPart], { type: "application/pdf" }));
      ...
      if (doc.createdBy) {
        ...
        const filename = `Damage, Part Loss, and Tool Penalty Commission Deduction Agreement - ${form.employeeName}.pdf`;
        await sendMessage({
          dmThreadId: thread.id,
          senderId: myProfileId,
          senderName: displayName || "Employee",
          body: `📄 Damage, Part Loss, and Tool Penalty Commission Deduction Agreement for ${form.employeeName} has been signed, and is ready for the employer/representative signature: [${filename}](${pdfUrl})`,
        });
      }
      ...
      getHrNotificationSettings()
        .then(({ taxForms }) => {
          ...
          void notifyHrRoleUsers(myProfileId, displayName || "Employee", excludeIds, `📄 Damage, Part Loss, and Tool Penalty Commission Deduction Agreement for ${form.employeeName} has been signed — the employer/representative signature is ready to be added.`);
        })
        .catch((err) => console.error("[damage] hr notify check failed:", err));

      setDoc({ ...doc, status: "signed", pdfUrl, formData: finalData as unknown as Record<string, any>, signatures: { employee: entry }, signedAt });
      void logActivity({ action: "damage_signed", targetType: "employee", targetLabel: form.employeeName });
      setSubmitted(true);
    } catch (err) {
