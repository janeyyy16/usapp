...
import { pushOfflineNotification } from "@/lib/partGovernance";
...
export function PartCollectionPage({ mod, sub }: { mod: ModuleDef; sub: SubModuleDef }) {
  ...
  // Item 3: Mark a truck-stock "Not Needed" part as Restock → status auto
  // changes to "Back in Stock" and a notification is fired.
  const handleMarkRestock = (uniqueId: string) => {
    const target = rows.find((r) => r.uniqueId === uniqueId);
    const nextRows = rows.map((row) =>
      row.uniqueId === uniqueId ? { ...row, collectType: "Restock", partStatusDesc: "Back in Stock" } : row
    );
    persistRows(nextRows);
    pushOfflineNotification({
      kind: "restock_auto",
      title: "Part back in stock",
      body: `${target?.partNo || "Part"} (Unique ID ${uniqueId}) on ticket ${target?.ticketNo || "(n/a)"} was marked Restock. Status auto-changed to "Back in Stock".`,
      ticketNo: target?.ticketNo ? String(target.ticketNo) : undefined,
    });
  };
                  ...
                  return (
                    <tr key={row.uniqueId}>
                      ...
                      <td>
                        ...
                        <button type="button" className="action-btn" style={{ marginLeft: 6, background: "rgba(34,197,94,0.85)", borderColor: "rgba(34,197,94,0.7)" }} onClick={() => handleMarkRestock(row.uniqueId)} title="Mark as Restock → status auto-changes to Back in Stock">
                          Restock
                        </button>
                      </td>
