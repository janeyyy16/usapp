/**
 * External Fill Valid ID — the no-login counterpart to FillValidIdPage.tsx.
...
export function ExternalFillValidIdPage({ docId }: Props) {
    ...
    (async () => {
      ...
      try {
        ...
        if (!document || document.documentType !== "valid_id_form") {
          ...
          setDoc(document);
          const existing = document.formData as Partial<ValidIdFormData>;
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const finalData: ValidIdFormData = { ...form, dateSigned: signedAt, signatureDataUrl: dataUrl };

      const pdfBlob = await captureHtmlToPdfBlob(
        buildValidIdFormBodyMarkup({ ...finalData, idPhotoUrls: idFiles.map((f) => URL.createObjectURL(f)) }, logoDataUrl, { name: form.employeeName, url: dataUrl, signedAt }),
        validIdFormStyles
      );
      ...
      const { pdfUrl } = await submitExternalSignature(docId, {
        ...
        attachments: { idPhotoUrls: idFiles },
      });
