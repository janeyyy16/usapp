...
export function OowTicketReport({ mod, sub }: Props) {
  ...
  const rows = useMemo(() => {
    ...
    if (!includeIw) r = r.filter(x=>!x.includeIw);
    if (schedStart) r = r.filter(x=>x.scheduleDate>=schedStart);
    if (schedEnd) r = r.filter(x=>x.scheduleDate<=schedEnd);
    if (compStart) r = r.filter(x=>x.completeDate && x.completeDate>=compStart);
    if (compEnd) r = r.filter(x=>x.completeDate && x.completeDate<=compEnd);
    return r;
  }, [compEnd, compStart, includeIw, location, schedEnd, schedStart, tech]);
