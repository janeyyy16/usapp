/**
 * External Sign Promotion Form — the no-login counterpart to
 * SignPromotionFormPage.tsx, mirroring ExternalSignDocumentPage.tsx exactly
 * (see that file's header comment for the full rationale: talks only to
 * /api/signable-documents, since an anonymous visitor has no Supabase
 * session for RLS to scope to, and only ever serves/accepts documents that
 * have no linked AHS profile).
 */
...
interface ExternalDoc {
  ...
  signatures: Partial<Record<PromotionSignatureSlot, { name: string; url: string; signedAt: string }>>;
  status: string;
...
export function ExternalSignPromotionFormPage({ docId }: Props) {
  ...
  const handleConfirmSign = async () => {
    ...
    try {
      ...
      const pdfBlob = await captureHtmlToPdfBlob(buildPromotionFormBodyMarkup(doc.formData, logoDataUrl, captureSignatures), promotionFormStyles);
          ...
          <div className="panel p-0 overflow-hidden">
            <div className="px-4 py-4 border-b border-white/10">
              ...
              <p className="text-[10px] text-muted-foreground mt-0.5">
                Review the form below, then sign as {SLOT_LABEL[doc.recipientSlot] ?? doc.recipientSlot}{doc.recipientName ? ` (${doc.recipientName})` : ""}.
              </p>
            </div>
            ...
            <div className="overflow-x-auto bg-white/5 p-4 flex justify-center">
              <div style={{ transform: "scale(0.78)", transformOrigin: "top center" }}>
                ...
                <div dangerouslySetInnerHTML={{ __html: buildPromotionFormBodyMarkup(doc.formData, logoDataUrl, doc.signatures) }} />
              </div>
            </div>
