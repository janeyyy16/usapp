...
import { uploadSignableDocumentSignature, uploadVehicleAgreementForm, refreshStorageAuthToken } from "@/lib/firebase/storage";
...
export function FillVehicleAgreementPage({ docId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const [pdfjsLib, bytes] = await Promise.all([import("pdfjs-dist/legacy/build/pdf.mjs"), loadBlankVehicleAgreementBytes()]);
