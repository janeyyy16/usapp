/**
 * Fill Driver's License — opened from the deep link a Team Messenger
 * message sends (see ReportHRDaily.tsx's "Driver's License" tab "Send
 * Request" flow). Standalone counterpart to the license field that used to
 * be embedded in Contractor Data Sheet / Master W-2 Technician & Office
 * Agreements — see driversLicenseFormTemplate.ts's header comment.
 * Single-party, same shape as FillSsnCardPage.tsx.
 */
...
async function compressForUpload(file: File): Promise<File> {
  try {
    ...
    console.error("[drivers-license] photo compression failed, uploading original:", err);
    return file;
  }
}
...
export function FillDriversLicensePage({ docId }: Props) {
    ...
    (async () => {
      ...
      try {
        ...
        if (!document || document.documentType !== "drivers_license_form") {
          ...
          setDoc(document);
          setForm((prev) => ({ ...prev, ...(document.formData as Partial<DriversLicenseFormData>) }));
        }
      } catch (err) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const compressedFiles = await Promise.all(licenseFiles.map(compressForUpload));
      const licensePhotoUrls = await withTimeout(
        Promise.all(compressedFiles.map((file, i) => uploadSignableDocumentAttachment(companyId, doc.id, "licensePhotoUrls", i, file))),
        60_000,
      ...
      const pdfUrl = await withTimeout(uploadDriversLicenseForm(companyId, form.employeeName, pdfBlob), 60_000, "Uploading document");
      ...
      if (doc.createdBy) {
        try {
          ...
          const filename = `Driver's License - ${form.employeeName}.pdf`;
          await sendMessage({
            dmThreadId: thread.id,
            senderId: myProfileId,
            senderName: displayName || "Employee",
            body: `📄 Driver's License for ${form.employeeName} has been submitted: [${filename}](${pdfUrl})`,
          });
        } catch (notifyErr) {
          console.error("[drivers-license] DM notify to creator failed:", notifyErr);
        }
      }
      ...
      getHrNotificationSettings()
        .then(({ taxForms }) => {
          ...
          void notifyHrRoleUsers(myProfileId, displayName || "Employee", excludeIds, `📄 Driver's License for ${form.employeeName} has been submitted.`);
        })
        .catch((err) => console.error("[drivers-license] hr notify check failed:", err));

      setDoc({ ...doc, status: "signed", pdfUrl, formData: finalData as unknown as Record<string, any>, signatures: { employee: entry }, signedAt });
      void logActivity({ action: "drivers_license_signed", targetType: "employee", targetLabel: form.employeeName });
      setSubmitted(true);
    } catch (err) {
