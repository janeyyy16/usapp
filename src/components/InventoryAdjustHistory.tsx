...
const ACTIONS=["Add","Remove","Correction","Transfer"];
...
const ALL=Array.from({length:40},(_,i)=>({id:i+1,uniqueId:"UID-"+pad(10000+i),partNo:"PT-"+pad(70000+i),description:pick(PARTS,i),location:pick(LOCATIONS.slice(1),i),adjustedAt:ds(-(i%30)-1),action:pick(ACTIONS,i),qty:(i%5)+1,by:"Admin",notes:""}));

export function InventoryAdjustHistory({mod,sub}:{mod:ModuleDef;sub:SubModuleDef}){
  ...
  const rows=useMemo(()=>{let r=ALL;if(location)r=r.filter(x=>x.location===location);if(startDate)r=r.filter(x=>x.adjustedAt>=startDate);if(endDate)r=r.filter(x=>x.adjustedAt<=endDate);if(partNo)r=r.filter(x=>x.partNo.toLowerCase().includes(partNo.toLowerCase()));if(uniqueId)r=r.filter(x=>x.uniqueId.toLowerCase().includes(uniqueId.toLowerCase()));return r;},[location,startDate,endDate,partNo,uniqueId]);
  return(<div className="min-h-screen flex flex-col"><main className="flex-1 max-w-[1600px] mx-auto w-full px-6 py-8">
    ...
    <div className="panel mb-4"><div className="flex flex-wrap items-end gap-3">
      <div className="flex flex-col gap-1 min-w-[160px] flex-1"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Location*</label>
        <button ref={locD.ref} onClick={()=>setLocOpen(o=>!o)} className="glass-input w-full text-sm py-1.5 px-3 rounded-md flex items-center justify-between gap-2"><span className={location?"":"text-muted-foreground"}>{location||"Select Location"}</span><Chev o={locOpen}/></button>
      ...
      <div className="flex flex-col gap-1"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Adjusted @</label>
    ...
    <div className="panel overflow-x-auto p-0"><table className="w-full text-sm"><thead><tr className="border-b border-white/10 bg-white/5">{["#","Unique ID","Part No","Description","Location","Adjusted @","Action","Qty","By","Notes"].map(h=><th key={h} className="px-3 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
    <tbody>{rows.length===0?<tr><td colSpan={10} className="px-4 py-12 text-center text-muted-foreground">No records found.</td></tr>:rows.map((r,idx)=><tr key={r.id} className={`border-b border-white/5 hover:bg-white/5 ${idx%2!==0?"bg-white/[0.02]":""}`}><td className="px-3 py-2.5 text-muted-foreground">{idx+1}</td><td className="px-3 py-2.5 font-mono text-xs text-blue-400">{r.uniqueId}</td><td className="px-3 py-2.5 font-mono text-xs">{r.partNo}</td><td className="px-3 py-2.5 text-xs">{r.description}</td><td className="px-3 py-2.5 text-xs">{r.location}</td><td className="px-3 py-2.5 text-xs text-muted-foreground">{r.adjustedAt}</td><td className="px-3 py-2.5"><span className={`px-2 py-0.5 rounded text-xs font-medium ${r.action==="Add"?"bg-green-500/20 text-green-300 border border-green-500/30":r.action==="Remove"?"bg-red-500/20 text-red-300 border border-red-500/30":r.action==="Transfer"?"bg-blue-500/20 text-blue-300 border border-blue-500/30":"bg-yellow-500/20 text-yellow-300 border border-yellow-500/30"}`}>{r.action}</span></td><td className="px-3 py-2.5 text-right">{r.qty}</td><td className="px-3 py-2.5 text-xs">{r.by}</td><td className="px-3 py-2.5 text-xs text-muted-foreground">{r.notes||"—"}</td></tr>)}</tbody></table></div>
  </main></div>);}
