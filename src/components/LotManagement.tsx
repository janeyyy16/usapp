...
const LOTS=Array.from({length:30},(_,i)=>({id:i+1,lotNo:"LOT-"+pad(15000+i),partNo:"PT-"+pad(70000+i),description:pick(PARTS,i),vendor:pick(VENDORS,i),location:pick(LOCATIONS.slice(1),i),qty:10+(i*3)%90,received:ds(-(i%45)),status:i%4===0?"Closed":"Active"}));

export function LotManagement({mod,sub}:{mod:ModuleDef;sub:SubModuleDef}){
  ...
  const rows=useMemo(()=>location?LOTS.filter(x=>x.location===location):LOTS,[location]);
  return(<div className="min-h-screen flex flex-col"><main className="flex-1 max-w-[1400px] mx-auto w-full px-6 py-8">
    ...
    <div className="panel mb-4"><div className="flex flex-wrap items-end gap-3">
      <div className="flex flex-col gap-1 flex-1 min-w-[200px]"><label className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Location</label>
    ...
    <div className="panel overflow-x-auto p-0"><table className="w-full text-sm"><thead><tr className="border-b border-white/10 bg-white/5">{["#","Lot No","Part No","Description","Vendor","Location","Qty","Received","Status"].map(h=><th key={h} className="px-3 py-3 text-left text-xs font-semibold text-muted-foreground uppercase tracking-wide whitespace-nowrap">{h}</th>)}</tr></thead>
    <tbody>{rows.map((r,idx)=><tr key={r.id} className={`border-b border-white/5 hover:bg-white/5 ${idx%2!==0?"bg-white/[0.02]":""}`}><td className="px-3 py-2.5 text-muted-foreground">{idx+1}</td><td className="px-3 py-2.5 font-mono text-xs text-blue-400">{r.lotNo}</td><td className="px-3 py-2.5 font-mono text-xs">{r.partNo}</td><td className="px-3 py-2.5 text-xs">{r.description}</td><td className="px-3 py-2.5 text-xs">{r.vendor}</td><td className="px-3 py-2.5 text-xs">{r.location}</td><td className="px-3 py-2.5 text-right">{r.qty}</td><td className="px-3 py-2.5 text-xs text-muted-foreground">{r.received}</td><td className="px-3 py-2.5"><span className={`px-2 py-0.5 rounded text-xs font-medium ${r.status==="Active"?"bg-green-500/20 text-green-300 border border-green-500/30":"bg-white/10 text-muted-foreground border border-white/15"}`}>{r.status}</span></td></tr>)}</tbody></table></div>
  </main></div>);}
