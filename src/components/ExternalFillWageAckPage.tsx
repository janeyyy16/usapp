/**
 * External Fill Acknowledgment of Wage — the no-login counterpart to
 * FillWageAckPage.tsx, opened from the link ReportHRDaily.tsx's "Send
 ...
 *
 * Same real-PDF overlay rendering as FillWageAckPage.tsx (identical field
 * rects). The PDF is built entirely client-side via the same pure
 * fillWageAckPdf used by the logged-in flow, then POSTed already-finished
 * to the server bridge, which uploads it and notifies HR — no DM step here
 * since there's no sender profile. Only the employee half is fillable
 * here — the employer/representative signature is always added afterward
 * by HR inside ReportHRDaily.tsx.
 */
...
// Same field rectangles as FillWageAckPage.tsx — see that file's header
...
export function ExternalFillWageAckPage({ docId }: Props) {
