import { INVENTORY_ROWS } from "@/lib/offlineData";
...
export function PartInventory({ mod, sub }: { mod: ModuleDef; sub: SubModuleDef }) {
  const companyId = "4930403"; const email = "offline@demo.com";
  ...
  const handleUseCrossInventory = useCallback(async (ownerBranch: string, partDesc: string) => {
    ...
    console.log("Cross-inventory (offline):", ownerBranch, partDesc);
  ...
  const handleShipSubmit = useCallback(() => {
    ...
    setShippingLog(prev => [{ ...shipForm, id: Date.now().toString(), shippedBy: "Offline User", createdAt: new Date().toLocaleDateString() }, ...prev]);
