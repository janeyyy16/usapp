...
import { LOCATIONS, USER_TYPES, SERVICE_TYPES_SS, pick, pad, todayStr, offsetStr } from "./shared";
...
export function DailyActivityReport({ mod, sub }: Props) {
  ...
  const [applied, setApplied] = useState({ location: "", startDate: todayStr(), endDate: offsetStr(1), targetUsers: [...TARGET_USERS] });

  const rows = useMemo(() => {
    let r = ALL_ROWS;
    if (applied.location) r = r.filter(x => x.location === applied.location);
    r = r.filter(x => applied.targetUsers.includes(x.userType));
    if (applied.startDate) r = r.filter(x => x.workDate >= applied.startDate);
    if (applied.endDate) r = r.filter(x => x.workDate <= applied.endDate);
    return r;
  }, [applied]);
    ...
    <main className="max-w-350 mx-auto px-4 py-6">
      ...
      <div className="panel mb-5">
        <div className="flex flex-wrap items-center gap-3">
          <div className="flex items-center gap-2 min-w-48 flex-1">
          ...
          <div className="flex items-center gap-1">
            ...
            <input id="dar-start" type="date" value={startDate} onChange={e => setStartDate(e.target.value)} title="Start date" placeholder="YYYY-MM-DD" className="glass-input text-sm py-1.5 px-2 rounded-md w-32.5 ml-2" />
            <select value={startTime} onChange={e => setStartTime(e.target.value)} title="Start time" aria-label="Start time" className="glass-input text-sm py-1.5 px-2 rounded-md">
              {TIMES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
            <span className="text-muted-foreground text-xs">~</span>
            <label htmlFor="dar-end" className="sr-only">End date</label>
            <input id="dar-end" type="date" value={endDate} onChange={e => setEndDate(e.target.value)} title="End date" placeholder="YYYY-MM-DD" className="glass-input text-sm py-1.5 px-2 rounded-md w-32.5" />
            <select value={endTime} onChange={e => setEndTime(e.target.value)} title="End time" aria-label="End time" className="glass-input text-sm py-1.5 px-2 rounded-md">
              {TIMES.map(t => <option key={t} value={t}>{t}</option>)}
            </select>
          </div>
          <div className="flex items-center gap-2 flex-1 min-w-48">
            <span className="text-xs font-semibold text-muted-foreground uppercase tracking-wide shrink-0 whitespace-nowrap">Target Users</span>
            <MultiCheckDropdown label="Target Users" options={TARGET_USERS} selected={targetUsers} onChange={setTargetUsers} />
          </div>
          <button onClick={() => setApplied({ location, startDate, endDate, targetUsers })} className="btn btn-primary flex items-center gap-2 px-5">
