/**
 * Fill Master W-2 Technician Comprehensive Policy, Consent & Agreement —
 * opened from the deep link ReportHRDaily.tsx's "Master W-2 Technician
 * Agreement" tab "Send Request" flow sends. HTML/CSS document (no source
 * PDF to overlay — see masterW2AgreementFormTemplate.ts's header comment),
 * rendered live as a preview of exactly what gets captured to PDF on
...
import {
  ...
  masterW2AgreementStyles,
  buildMasterW2AgreementBodyMarkup,
  type MasterW2AgreementFormData,
} from "@/lib/masterW2AgreementFormTemplate";
...
const BLANK_FORM: MasterW2AgreementFormData = {
  employeeId: "",
  employeeName: "",
  firstName: "",
  middleName: "",
...
export function FillMasterW2AgreementPage({ docId }: Props) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        setLogoDataUrl(logo);
        if (!document || document.documentType !== "master_w2_agreement") {
          setError("This document doesn't exist or has been removed.");
        } else {
          setDoc(document);
          const existing = document.formData as Partial<MasterW2AgreementFormData>;
  ...
  const updateField = <K extends keyof MasterW2AgreementFormData>(key: K, value: MasterW2AgreementFormData[K]) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const pdfBlob = await captureHtmlToPdfBlob(buildMasterW2AgreementBodyMarkup(finalData, logoDataUrl), masterW2AgreementStyles);
      const pdfUrl = await uploadMasterW2AgreementForm(companyId, employeeName, pdfBlob);
      ...
      if (doc.createdBy) {
        ...
        await sendMessage({
          ...
          body: `📋 Master W-2 Technician Comprehensive Policy, Consent & Agreement for ${employeeName} has been signed, and is ready for the employer signature: ${pdfUrl}`,
        });
      }
      ...
      getHrNotificationSettings()
        .then(({ taxForms }) => {
          ...
          void notifyHrRoleUsers(myProfileId, displayName || "Employee", excludeIds, `📋 Master W-2 Technician Comprehensive Policy, Consent & Agreement for ${employeeName} has been signed — the employer signature is ready to be added.`);
        })
        .catch((err) => console.error("[master-w2-agreement] hr notify check failed:", err));

      setDoc({ ...doc, status: "signed", pdfUrl, formData: finalData as unknown as Record<string, any>, signatures: { employee: entry }, signedAt });
      void logActivity({ action: "master_w2_agreement_signed", targetType: "employee", targetLabel: employeeName });
      setSubmitted(true);
    } catch (err) {
        ...
        {loading ? (
          ...
          <div className="panel p-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-3">
              <div>
                <label className="text-[10px] font-semibold text-muted-foreground uppercase tracking-wide mb-1 block">Branch</label>
                <select className="glass-input text-sm py-1.5 px-3 rounded-md w-full" value={form.branch} onChange={(e) => updateField("branch", e.target.value)}>
                  ...
                  {MASTER_W2_AGREEMENT_BRANCHES.map((b) => <option key={b} value={b}>{b}</option>)}
                </select>
              </div>
              <div>
            ...
            <div className="overflow-x-auto bg-white/5 rounded-md p-4 flex justify-center">
              <style dangerouslySetInnerHTML={{ __html: masterW2AgreementStyles }} />
              <div dangerouslySetInnerHTML={{ __html: buildMasterW2AgreementBodyMarkup(form, logoDataUrl) }} />
            </div>
