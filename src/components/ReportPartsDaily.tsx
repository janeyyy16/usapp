...
export function ReportPartsDaily({mod,sub}:{mod:ModuleDef;sub:SubModuleDef}){
  ...
  const handleExportCSV = () => {
    exportToCSV("parts_daily_report",
      ["Branch","Name","Role","Collections","Pending RA","RA Created","Receives","Warnings"],
      filtered.map((a:any)=>[a.branch,a.name,a.role,a.collections,a.pendingRA,a.raCreated,a.receives,a.warnings])
    );
  };
  ...
  return(
    <div className="min-h-screen flex flex-col"><main className="flex-1 max-w-[1600px] mx-auto w-full px-6 py-8">
      ...
      <div className="panel overflow-x-auto p-0">
        <div className="px-4 py-3 border-b border-white/10 font-semibold text-sm flex justify-between"><span>Staff Detail</span><span className="flex items-center gap-2 text-xs text-muted-foreground">{filtered.length} staff<button onClick={handleExportCSV} title="Download CSV" className="inline-flex items-center justify-center h-7 w-7 rounded hover:bg-white/10 text-muted-foreground hover:text-foreground transition-colors"><svg className="h-4 w-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="7 10 12 15 17 10"/><line x1="12" y1="15" x2="12" y2="3"/></svg></button></span></div>
