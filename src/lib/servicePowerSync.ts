...
// USPS ZIP3-prefix -> state ranges (standard, stable reference data — the
// first 3 digits of a US zip code determine the state in all but a
// handful of edge cases we don't need to special-case here). Used to
// guard the loose city-name fallback in resolveBranchFromCustomer below
// against matching a same-named city in the wrong state (e.g. Villa
// Ridge, MO vs Villa Ridge, IL — real, ~250mi-apart towns that share a
// name, which the old unguarded match confused).
const ZIP3_STATE_RANGES: Array<[number, number, string]> = [
  ...
  [680, 693, 'NE'], [700, 714, 'LA'], [716, 729, 'AR'], [730, 731, 'OK'],
  [732, 749, 'OK'], [733, 733, 'TX'], [750, 799, 'TX'], [800, 816, 'CO'],
...
function stateFromZip5(zip5: string): string {
...
export function resolveBranchFromCustomer(consumer: any): string {
  ...
  if (rawCity) {
    ...
    // Loose match against zip coverage table — but only accept an entry
    // whose OWN zip5 is in the same state as the customer's. Without this,
    // a same-named city in a different state (e.g. Villa Ridge, MO vs
    // Villa Ridge, IL, ~250mi apart) silently matches whichever one
    // happens to be in the table, sending the ticket to the wrong branch.
    // If the customer's state can't be determined at all, fall back to
    // matching on city name alone (the old behavior) rather than giving up.
    const customerState = normalizeStateCode(rawState);
    for (const [zip5Key, entry] of Object.entries(ZIP_COVERAGE)) {
      if (String(entry.city).trim().toLowerCase() !== rawCity) continue;
      if (customerState) {
        const entryState = stateFromZip5(zip5Key);
        if (entryState && entryState !== customerState) continue;
      }
