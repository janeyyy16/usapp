-- Which zone interview_time is in (CST or EST — same two zones
-- profiles.schedule_timezone already uses for Required Schedule), set
-- alongside the time itself from the "Interviewing" status dialog
...
-- Adding a 6th parameter changes the function's signature — Postgres would
-- otherwise keep the old 5-arg version around as a SEPARATE overload
-- (defaults don't merge signatures), and a plain 5-arg RPC call would then
-- be ambiguous between the two. Drop the old signature explicitly first.
drop function if exists hr_update_candidate_status(uuid, text, date, date, text);
